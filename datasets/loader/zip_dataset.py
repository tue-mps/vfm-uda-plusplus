...
class ZipDataset(torch.utils.data.Dataset):
    def __init__(
            ...
            accepted_target_list_txt: str = None,
    ):
        ...
        self.accepted_target_list_txt = accepted_target_list_txt
        ...
        if self.accepted_target_list_txt is not None:
            with open(self.accepted_target_list_txt) as file:
                accepted_targets = [line.rstrip('\n') for line in file]
            accepted_targets = [x.split(' ')[0] for x in accepted_targets]
            accepted_targets = set(accepted_targets)
            tmp_filtered = {}
            for image_name, m in self.image_folder_members.items():
                if image_name in accepted_targets:
                    tmp_filtered[image_name] = m
            self.image_folder_members = tmp_filtered
